...
def main():
    ...
    if nb_uuid and target_payload["api_endpoint_type"] == "NETWORK_BROKER":
        # Root level assignment for Network Broker UUID
    ...
    # 3. Multi-turn config (Properly nested inside connection_params!)
    if session_supported:
        mt_config = parse_json_env("MULTI_TURN_CONFIG")
        if mt_config and "type" in mt_config:
            target_payload["connection_params"]["multi_turn_config"] = mt_config
    ...
    # 5. Context blocks (Parsed correctly as JSON dicts)
